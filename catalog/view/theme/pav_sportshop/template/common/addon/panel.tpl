...
$headerlayouts = array('default'=>'Default', 'menusearch' => 'Main Menu - Search', 'hassearch'=>'Vertical Menu - Search'); 
			...
			<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="clearfix">
				...
				<p class="group-input">
					...
					<select name="userparams[productlayout]">
						<?php 
						foreach( $productlayouts as $k=>$skin ) {  $skin = basename($skin);
							$selected = '';
							if(isset($_COOKIE[$themeName.'_productlayout'])) {
								if($_COOKIE[$themeName.'_productlayout'] == $k){
									$selected = "selected='selected'";
								}
							}
						?>
						<option value="<?php echo ($k);?>" <?php echo $selected; ?> ><?php echo ($skin);?></option>
						<?php } ?>
					</select>					
				</p>
			                              	...
			                              	<?php foreach ($group['selector'] as $item ) {  ?>
												 <?php  if (isset($item['type'])&&$item['type']=="image") { ?>
												  <div class="form-group background-images"> 
														...
														<a class="clear-bg btn btn-default btn-outline-small" href="#">Clear</a>
				                                  ...
				                                  <?php } elseif( $item['type'] == "fontsize" ) { ?>
				                                   <div class="form-group">
													...
													<option value="<?php echo $fs; ?>"><?php echo $fs; ?></option>
															...
															<a href="#" class="clear-bg btn btn-default btn-outline-small">Clear</a>
				                                  </div>
				                                  <?php } else { ?>
				                                  <div class="form-group">
														<label><?php echo $item['label']?></label>
														<input value="" size="10" name="customize[<?php echo $group['match']?>][]" data-match="<?php echo $group['match']?>" type="text" class="input-setting" data-selector="<?php echo $item['selector']?>" data-attrs="<?php echo $item['attrs']?>"><a href="#" class="clear-bg btn btn-default btn-outline-small">Clear</a>
												  </div>
				                                  <?php } ?>
