...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
                ...
                if (isset($this->request->post['tab_data'])) {
                    $data['tabs'] = $this->request->post['tab_data'];
                } elseif (isset($this->request->get['product_id'])) {
                    $data['tabs'] = $this->model_catalog_product->getTotalTabs($this->request->get['product_id']);
                } else {
                     $data['tabs'] = array();
                }
                if (isset($this->error['tabs'])) {
                    $data['error_tabs'] = $this->error['tabs'];
                } else {
                    $data['error_tabs'] = '';
                }
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			...
			if ((utf8_strlen($value['meta_title']) < 3) || (utf8_strlen($value['meta_title']) > 255)) {
                ...
                //START EXTRA PRODUCT TABS
                if(isset($this->request->post['tab_data'])){
                    foreach ($this->request->post['tab_data'] as $tabs) {
                        foreach ($tabs as $language_id => $value) {
                            if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 255)) {
                                $this->error['tabs'][$value['tab_row']][$language_id] = $this->language->get('error_tab');
                            }
                        }
                    }
                }
